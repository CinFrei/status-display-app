...
X $DISPLAY $EXTRA_X_ARGS -config /xorg-dummy.conf &
PID_X=$!
export DISPLAY
...
/usr/bin/x11vnc -safer -passwd "$VNC_PASSWORD" -forever -quiet -scale 1 -display "$DISPLAY" -notruecolor -shared -geometry "$WINDOW_SIZE_VNC" $EXTRA_VNC_ARGS &
PID_VNC=$!
...
echo ""
su -c "$CHROMIUM_CMD" "$DESKTOP_USER" &
PID_CHROMIUM=$!
